...
class  AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if ($this->isDefaultCredentialBlocked($user, $password)) {
            throw ValidationException::withMessages([
                'email' => __('Default credentials are disabled on non-local environments. Please reset this account password before login.')
            ]);
        }
    ...
    private function isDefaultCredentialBlocked(User $user, string $password): bool
    {
        ...
        $defaults = [
            ['email' => 'admin@resultchecker.com', 'password' => 'admin123'],
            ['email' => 'user@example.com', 'password' => 'user12345'],
        ];

        foreach ($defaults as $default) {
            if (strtolower($user->email) === strtolower($default['email']) && $password === $default['password']) {
                return true;
            }
        }
     ...
     * @since 1.0
     */
    public function authVerifyCaptcha()
    {
        $auth = Auth::check();
        ...
        if (!$auth) {
            ...
            $this->settingsService->updateHcheck($clear);
            return redirect()->route('auth.login.form');
        }

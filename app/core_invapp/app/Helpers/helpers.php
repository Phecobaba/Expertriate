...
if (!function_exists("get_etoken")) {
    ...
    function get_etoken($val = null, $cache = false)
    {
        ...
        if ($cache===true) {
            ...
            if (empty($token)) {
                $site = gss('sit' . 'e_me'.'rch' .'an'. 'di' .'se', []);
                $app = gss('app'.'_ac'.'qui'.'re', []);
                $site = is_array($site) ? $site : [];
                $app = is_array($app) ? $app : [];
                $str = array_merge($site, $app);
...
if (!function_exists('get_fx_rate')) {
     ...
     * @since 1.0
     */
    function get_fx_rate($form, $to, $amount = 1, $flush = false)
    {
        $form = strtoupper($form);
        $to = strtoupper($to);
        $form = (in_array($form, ['DEFAULT', 'BASE'])) ? base_currency() : $form;
        $form = (in_array($form, ['ALTER', 'SECONDARY'])) ? secondary_currency() : $form;

        $to = (in_array($to, ['DEFAULT', 'BASE'])) ? base_currency() : $to;
        $to = (in_array($to, ['ALTER', 'SECONDARY'])) ? secondary_currency() : $to;

        $rates = actived_exchange_rates();
        $decimal = is_crypto($to) ? dp_calc('crypto') : dp_calc('fiat');

        if (isset($rates[$form]) && isset($rates[$to])) {
            $rate   = ($amount / $rates[$form]);
            $amount = $rates[$to] * $rate;
...
if (!function_exists('global_meta_content')) {
    ...
    function global_meta_content($type = null)
    {
        ...
        if ($type == 'share') {
            ...
            $desc = (sys_settings('seo_description')) ? '<meta name="description" content="' . sys_settings('seo_description') . '">' : '';
...
if (!function_exists('cipher_id')) {
    ...
    {
        $hc = gss('hea'.'lth_'.'chec'.'ker', 0);
        return (!empty($hash)) ? $hash.$hc : $hc;
    }
}
...
if (!function_exists('serverOpenOrNot')) {
    ...
    function serverOpenOrNot($site)
    {
        ...
        if (empty($url) || empty($server)) return false;

...
class HealthCheckService extends Service
{
    ...
    public function checkSystem($flush = false) {
        // Never block local development with remote health/license probes.
        ...
        if (($hs && $tc <= time()) || $flush) {
            ...
            if ($cpc && $cps && (strlen($sys) > 11)) {
                try {
                    ...
                    $http = Http::timeout(5)->connectTimeout(3)->get('htt' .'ps://ap'.$htt.'om/'.$this->uri.'7'. 'd5b' .'16c6', $data);
                    if ($http->successful()) {
                ...
                upss('payo'.'ut'. '_' .'che'.'ck', (time() + 1800)); session()->put('sy'.'st'.'em'. '_' .'err'.'or', true);
    ...
    public function updateSystem()
    {
        ...
        $siteMerch = gss('si'.'te'.'_me'.'rchan'.'dise', []);
        $siteMerch = is_array($siteMerch) ? $siteMerch : [];
        clear_ecache(); Cache::put(get_m5host(), array_merge($siteMerch, $upt), $addmin);
    }

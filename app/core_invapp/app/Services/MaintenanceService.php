...
class MaintenanceService extends Service
{
    ...
    public function getInstaller()
    {
        ...
        if (empty(gss('installed_apps'))) {
            try {
                $res = Http::timeout(5)->connectTimeout(3)->post($this->validSeg.'/ch' . 'eck/in' .'sta'. 'ller', $this->validData());
    ...
    public function fixBaseURL()
    {
        ...
        if (get_path() != $base) {
            try {
                ...
                $res = Http::timeout(5)->connectTimeout(3)->post($this->validSeg.'/che' . 'ck/up'.'da'.'ter', $this->validData($new));
                upss('baseurl_apps', get_path());
            } catch (\Exception $e) {}
        }

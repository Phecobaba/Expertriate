...
<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body modal-body-md">
            <h4 class="title">{{ blank($userAccount) ? __('Add Wallet Address') : __('Update Wallet Address') }}</h4>
            @if(blank($userAccount))
            <p>{{ __('Add your wallet address to withdraw your funds.') }}</p>
            @else 
            <p>{{ __('Update your existing wallet address for future withdraw.') }}</p>
            @endif
            ...
            <form action="{{$actionUrl}}" method="POST" class="form">
                <div class="row gy-3">
                        ...
                        <div class="row gx-1">
                            <div class="col-8 col-md-9">
                                <div class="form-group">
                                    <label class="form-label" for="paypal-email">{{ __('Wallet Address') }} <span class="text-danger">*</span></label>
                                    <div class="form-control-wrap">
                                        <input type="text" name="wdm-email" value="{{ data_get($userAccount, 'config.email') }}" class="form-control form-control-lg" id="paypal-email" placeholder="{{ __('Enter email connected to wallet') }}">                                    
                                    </div>
                                </div>
                            </div>
                            <div class="col-4 col-md-3">
                                @if(!blank($currencies))
                                <div class="form-group">
                                    <label class="form-label" for="paypal-currency">{{ __('Wallet Type') }}</label>
                                    <div class="form-control-wrap">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <label class="form-label" for="account-label">{{ __('Label') }} <span>{{ __('(Required)') }}</span></label>
                            <div class="form-control-wrap">
                                <input type="text" name="wdm-label" value="{{ data_get($userAccount, 'name') }}" class="form-control form-control-lg" id="account-label" placeholder="eg. Bitcoin, Ethereum">
                            </div>
                            <div class="form-note">

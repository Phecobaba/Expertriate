            ...
            <div class="nk-block-between gx-2">
                ...
                <div class="nk-block-head-content">
                    <ul class="nk-block-tools g-3">
                        <li>
                            <a href="{{ route('admin.investment.list') }}" class="btn btn-primary d-none d-sm-inline-flex"><em class="icon ni ni-invest"></em><span>{{ __("Invested Plans") }}</span></a>
                        ...
                        @if(has_route('admin.investment.scheme.save'))
                        <li>
...
<script type="text/javascript">
    const quick_update = "{{ route('admin.investment.scheme.status') }}",
        ...
        qmsg = { title: "{{ __('Are you sure?') }}", btn: {cancel: "{{ __('Cancel') }}", confirm: "{{ __('Confirm') }}"}, context: "{!! __("Do you want to perform this action?") !!}", action: {active: "{!! __("Do you want to active the investment scheme so users can purchase and invest on this plan?") !!}", inactive: "{!! __("Do you want to inactive the investment scheme? Once you confirmed, users cannot purchase the plan anymore.") !!}", archived: "{!! __("Do you want to archived the scheme? Once you confirmed, the plan will be hide cannot purchase the plan anymore.") !!}"} };
</script>
@endpush

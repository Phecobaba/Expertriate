                ...
                <form action="{{ route('admin.save.app.settings') }}" class="form-settings" method="POST">
                    ...
                    <div class="form-sets gy-3 wide-md">
                        <div class="row g-3 align-start">
                            <div class="col-lg-5">
                                <div class="form-group">
                                    <label class="form-label" for="market-data-provider">{{ __('Market Data Provider') }}</label>
                                    <span class="form-note">
                                        {{ __('Select which provider powers dashboard market widgets.') }}<br>
                                        {{ __('Use CoinGecko for keyless mode, or add an API key for premium providers.') }}
                                    </span>
                                </div>
                            </div>
                            <div class="col-lg-7">
                                <div class="form-group">
                                    <div class="form-control-wrap">
                                        <select class="form-select" id="market-data-provider" name="market_data_provider">
                                    ...
                                    <div class="form-control-wrap mt-2">
                                        <input type="text" class="form-control" id="market-data-api-key" name="market_data_api_key" value="{{ sys_settings('market_data_api_key') }}" placeholder="{{ __('Provider API Key (optional for CoinGecko)') }}">
                                    </div>
                                    <div class="form-control-wrap mt-2">
                                        <input type="url" class="form-control" id="market-data-base-url" name="market_data_base_url" value="{{ sys_settings('market_data_base_url') }}" placeholder="{{ __('Custom API Base URL (optional)') }}">
                                    </div>
                                    <div class="form-note mt-2">
                                        <span class="text-primary">{{ __('Runtime status:') }}</span>
                                        @if(sys_settings('market_data_error_msg'))
                                            <span class="text-danger">{{ __('Fallback mode') }}</span>
                                            <span class="d-block">{{ sys_settings('market_data_error_msg') }}</span>
                                        @else
                                            <span class="text-success">{{ __('Live feed active or no recent provider errors.') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
